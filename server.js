...
app.post('/generate', async (req, res) => {
    ...
    logger(debugMode, `Request received for /generate with URL: ${url}`);
    logger(debugMode, `Prompt: ${JSON.stringify(prompt, null, 2)}`);
    ...
    try {
        ...
        logger(debugMode, 'Response from external API received');
        ...
        response.body.on('data', chunk => {
            logger(debugMode, 'Chunk received:', chunk.toString());
        });

        response.body.on('end', () => {
            logger(debugMode, 'Response streaming completed');
        });

    } catch (error) {
        logger(debugMode, 'Error:', error);
